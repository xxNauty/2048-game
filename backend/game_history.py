...
def generate_report(game_settings, count_up, count_down, count_left, count_right, status, max_value_on_gameboard):
    ...
    file_name = GAME_HISTORY_PATH + date_of_game.strftime(os.getenv("DATE_FORMAT_FOR_FILENAMES")) + ".json"
...
def read_history_files():
    files = [file for file in os.listdir(GAME_HISTORY_PATH) if os.path.isfile(os.path.join(GAME_HISTORY_PATH, file))]

    for ignore_file in ["example.json", ".gitignore"]:
